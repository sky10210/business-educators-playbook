# Business Educator's Playbook Build Status

## Production-complete Cornerstone strategies

- ECS-001 Think–Pair–Share
- ECS-002 Gallery Walk
- ECS-003 Four Corners
- ECS-004 Jigsaw
- ECS-005 Fishbowl
- ECS-006 Station Rotation
- ECS-007 Speed Networking
- ECS-008 Round Robin
- ECS-009 Shark Tank
- ECS-010 Case Study Teams

## Production-complete supplemental strategies

- SUP-001 Case Study Carousel

## Shared foundation

- Teacher Language Library
- Universal Discussion Rubric
- Timing Templates

## Repository quality

- GitHub Pages deployment workflow
- Automated metadata validation
- Duplicate ID and slug detection
- Internal link checking
- Responsive strategy-page framework
- Custom 404 page
- `.nojekyll` support


Business Simulations
- SIM-001 Full Business Simulation


## Production-complete Business Simulations

- BS-001 Consulting Firm

## Master Repository Package

This package consolidates the cumulative website build through BS-002 Board Meeting for direct GitHub repository upload.

- BS-003 Product Launch


## Platform Updates

- Homepage v2.9: publisher-style hero, quick-needs panel, advanced search, course/category/time filters, responsive strategy cards, and stronger calls to action.


## Platform v3.0  -  Vivid Lessons Complete Redesign
- Rebuilt the homepage, coach, about page, and all strategy pages in the Vivid Lessons visual system.
- Added a permanent desktop sidebar, responsive mobile navigation, section top navigation, strategy tables of contents, print styling, and corrected relative asset paths.


## v3.3 Activity Finder Rebuild

- Rebuilt `coach.html` as the Activity Finder.
- Reduced hero and interface typography.
- Added dropdowns for student action, grouping, class length, energy, teaching need, and course.
- Added automatic pathway highlighting and smooth navigation.
- Added five color-coded pathway sections and sixteen teacher-facing pathway cards.
- Replaced remaining Activity Finder wording with Activity Finder.
- Preserved full repository structure for drag-and-drop GitHub replacement.


## v3.4 Business Simulations Complete

The Business Simulations section now contains ten publication-ready strategies:

- BS-001 Consulting Firm
- BS-002 Boardroom Meeting
- BS-003 Crisis Management Team
- BS-004 Investor Pitch Panel
- BS-005 Product Launch Simulation
- BS-006 Executive Decision Matrix
- BS-007 Business Negotiation
- BS-008 Press Conference Simulation
- BS-009 Company Turnaround
- BS-010 Hiring Committee

Every page includes a classroom trailer, classroom snapshot, quick scan, instructional value, preparation, roles, timed phases, teacher language, course examples, problems and fixes, explained variations, assessment, reflection, companion-resource roadmap, related strategies, and color-coded section hierarchy.


## v3.5 Discussion and Decision Structures Complete

Completed six production-ready strategies:

- DP-001 Silent Discussion
- DP-002 Socratic Seminar for Business
- DP-003 Chalk Talk
- DP-004 Structured Academic Controversy
- DP-005 Decision Matrix
- DP-006 Affinity Mapping

This build also includes a repository-wide navigation audit covering internal page links, fragments, buttons, query-based library links, and print actions.


## v3.6a Presentation and Communication Part 1

Completed four publication-ready strategies:

- PC-001 TED-Style Business Talk
- PC-002 Trade Show Exhibition
- PC-003 Executive Briefing
- PC-004 Sales Presentation

This is a smaller drag-and-drop release intended to keep each build stable and easy to validate.
